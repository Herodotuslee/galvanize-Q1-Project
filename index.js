    ...
    requests: [
      {
        ...
        features: [label, landmark, webDetection]
      }
    ]
  };
  ...
  fetch(`https://vision.googleapis.com/v1/images:annotate?key=${key}`, {
    ...
    .then(data => {
      ...
      for (let i = 0; i < data.responses[0].labelAnnotations.length; i++) {
        document.querySelector(".labels-Result").innerHTML += `${
          data.responses[0].labelAnnotations[i].description
        }<hr>`;
      }
      ...
      if (data.responses[0].landmarkAnnotations) {
        ...
        document.querySelector(".landmark-Result").innerHTML = `${
          data.responses[0].landmarkAnnotations[0].description
        }<hr>`;

        let lat =
          data.responses[0].landmarkAnnotations[0].locations[0].latLng.latitude;
        let lng =
          data.responses[0].landmarkAnnotations[0].locations[0].latLng
            .longitude;
      ...
      for (
        let i = 0;
        i < data.responses[0].webDetection.pagesWithMatchingImages.length;
        i++
      ) {
        let webUrl =
          data.responses[0].webDetection.pagesWithMatchingImages[i].url;
        let webName = data.responses[0].webDetection.webEntities[i].description;
        document.querySelector(
          ".WebSite-Result"
        ).innerHTML += `<a href= ${webUrl}>${webName}</a><hr>`;
      }
    });
  console.log(url);
